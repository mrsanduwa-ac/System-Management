// ⚠️ Replace this URL with your own new Google Apps Script Web App URL
const WEB_APP_URL = "https://mrsanduwa-ac.github.io/System-Management/";
...
async function validateAndLoadApp(passcode) {
    ...
    try {
        ...
        if (!response.ok) throw new Error("Network error. Please check the Web App URL and permissions.");
...
function attachEventListeners() {
    ...
    el.scannedOrderBarcodesList.addEventListener("click", e => {
        if (e.target.classList.contains("delete-barcode-btn")) {
...
// --- Barcode and Session Management ---
...
function loadSelectedSession() {
    ...
    if (!sessionId) return Swal.fire({ title: "Oops...", text: "Please select a session to load.", icon: "warning" });
...
function renderPermanentBarcodes() {
    ...
    el.noOrderBarcodesMessage.style.display = scannedUniqueBarcodes.size > 0 ? "none" : "block";
    if (scannedUniqueBarcodes.size > 0) {
        Array.from(scannedUniqueBarcodes).reverse().forEach(barcode => {
